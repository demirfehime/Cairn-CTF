...
def write_graph_snapshot_reference(
    ...
    *,
    ...
    snapshot_dir = f"{phase}-{uuid.uuid4().hex[:12]}"
    configured_root = Path(GRAPH_SNAPSHOT_ROOT)
    workspace_root = Path(container_name)
    if configured_root.is_absolute():
        path = str(configured_root / snapshot_dir / "graph.yaml")
    elif workspace_root.is_absolute():
        # On Windows, POSIX-style /tmp is not an absolute Windows path. Keep
        # snapshots inside the per-project workspace so the host Agent can read them.
        path = str(workspace_root / ".cairn-prompts" / snapshot_dir / "graph.yaml")
    else:
        # Container names are not host paths; retain the in-container POSIX location.
        path = f"{GRAPH_SNAPSHOT_ROOT}/{snapshot_dir}/graph.yaml"
...
def run_worker_process(
    ...
    *,
    ...
    process_kwargs: dict[str, object] = {"timeout_seconds": timeout_seconds}
    if stdin_text is not None:
        process_kwargs["stdin_text"] = stdin_text
    ...
    try:
        ...
        try:
            ...
            LOG.warning("Could not collect extension trace", exc_info=True)
        if lease is not None:
