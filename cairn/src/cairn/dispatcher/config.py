...
ExecutionMode = Literal["container", "local", "api"]
...
class RuntimeConfig(BaseModel):
    ...
    server_managed_agents: bool = False
...
class DispatchConfig(BaseModel):
    ...
    def validate_workers(self) -> "DispatchConfig":
        ...
        if not self.workers and not self.runtime.server_managed_agents:
    ...
    def validate_execution_mode(self) -> "DispatchConfig":
        ...
        elif self.runtime.execution == "local":  # local reuses the host CLI config
            if self.local is None:
                self.local = LocalConfig()
        else:  # api: host CLI runtime with server-managed API credentials
            if self.local is None:
                self.local = LocalConfig()
            for worker in self.workers:
                required = WORKER_ENV_KEYS[worker.type]
