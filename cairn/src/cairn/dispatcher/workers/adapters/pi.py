...
class PiDriver(WorkerDriver):
    ...
    def __init__(self, local: bool = False, api: bool = False):
        self.local = local
        self.api = api

    def local_binary(self) -> str | None:
        if self.api:
            from cairn.dispatcher.workers.adapters.pi_runner import bundled_cli
    ...
    def build_execute(self, worker: WorkerConfig, prompt: str, session: str | None) -> DriverResult:
        if self.api:
            return self._api_execute(worker, prompt, session)
        if self.local:
    ...
    def _api_execute(self, worker: WorkerConfig, prompt: str, session: str | None) -> DriverResult:
        # The host runner writes configuration inside the project and passes long
        # prompts through stdin, without a POSIX shell or secrets in process args.
        return DriverResult(
            argv=[sys.executable, "-m", "cairn.dispatcher.workers.adapters.pi_runner"],
            session=session,
            stdin_text=json.dumps({"prompt": prompt, "session": session, "agent_dir": self._agent_dir(worker).split("/")[-2:]}),
        )
    ...
    def _agent_dir(worker: WorkerConfig) -> str:
        project_id = worker.env.get("CAIRN_PROJECT_ID", "unscoped")
        assignment_epoch = worker.env.get("CAIRN_ASSIGNMENT_EPOCH", "0")
        agent_id = worker.agent_id or worker.env.get("CAIRN_AGENT_ID") or worker.name
        label = re.sub(r"[^A-Za-z0-9_.-]+", "-", worker.name).strip("-") or "agent"
        identity = f"{project_id}:{agent_id}:{assignment_epoch}"
        digest = hashlib.sha256(identity.encode("utf-8")).hexdigest()[:12]
        return str(
            PurePosixPath("/tmp/cairn-pi")
            / f"{label}-{digest}"
