...
# Rules
- This is an isolated project workspace. Use only the supplied graph snapshot and this project's files. Never inspect sibling or parent directories, launcher databases, other project runs, browser history, or another project's API/export. If context is missing, report the gap instead of searching for another project.
...
- Treat this as a bounded work cycle: stop before the execution budget expires and return the latest confirmed incremental result. Do not wait indefinitely for total Goal completion.
