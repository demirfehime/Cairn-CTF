...
class ContainerManager:
    ...
    def set_project_identity(self, project_id: str, identity: str) -> None:
        # Docker container names already isolate projects. The database-generated
        # project id remains the container identity for this backend.
        return None
    ...
    def build_exec_process(
        ...
        import json
        extensions = json.loads(env.get("CAIRN_EXTENSIONS_JSON", "{}"))
        if extensions.get("skill_paths") or any(server.get("enabled", True) for server in extensions.get("mcp_servers", {}).values()):
            raise ValueError("Configured Skills/MCP require local or API execution mode")
        if stdin_text is not None:
            raise ValueError("container execution does not support stdin_text")
    ...
    def write_text_file(self, container_name: str, path: str, content: str) -> None:
        self.write_bytes_file(container_name, path, content.encode("utf-8"))
    ...
    @staticmethod
    def _text_file_archive(path: str, content: str | bytes) -> tuple[str, bytes]:
